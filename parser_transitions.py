    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            self.buffer.pop(0)
             ...
             to_right = self.stack[-2]
             self.stack.pop(-2)
             to_left = self.stack[-1]
             self.dependencies.append(tuple((to_left,to_right)))
        elif transition == 'RA':
            to_right = self.stack[-1]
            self.stack.pop(-1)
            to_left = self.stack[-1]
            self.dependencies.append(tuple((to_left,to_right)))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        b_trans = model.predict(parsers)
        for pp, transition in zip(parsers, b_trans):
            pp.parse([transition])
